...
(function () {
    ...
    if (!luxon) {
        throw "jellog/luxon library requires the luxon library included to the page!";
    }

    /* TIMING *************************************************/

    jellog.timing = jellog.timing || {};
    ...
    jellog.timing.convertFieldsToIsoDate = function (form, fields) {
        for (var field of fields) {
            var dateTime = luxon.DateTime
                .fromFormat(
                    getObjectValue(form, field),
                    jellog.localization.currentCulture.dateTimeFormat.shortDatePattern,
                    {locale: jellog.localization.currentCulture.cultureName}
                );

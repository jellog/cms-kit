(function ($) {
    ...
    var successModal = new jellog.ModalManager({
        viewUrl: jellog.appPath + "Public/Newsletters/SuccessModal",
        scriptUrl: "/Pages/Public/Newsletters/successModal.js",
        modalClass: "newsletterSuccessModal"
    });
    
    jellog.widgets.CmsNewsletter = function ($widget) {
        ...
        function init() {
            $widget.find(form).on('submit', '', function (e) {
                ...
                dataGap.cmsKit.public.newsletters.newsletterRecordPublic.create(createPayload).then(function () {
                    successModal.open({
                        emailAddress: formAsObject.newsletterEmail,
                        preference: preference,
                        source: dataSourceValue,
